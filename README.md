...
- **Overlap view** — two things one project buys you:
  ...
  - **HIPAA Security Rule crosswalk** — which HIPAA standards your 800-171 work supports, derived by chaining [800-171 Appendix D](https://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-171r2.pdf) (171→800-53) with [NIST SP 800-66 Rev 2](https://csrc.nist.gov/pubs/sp/800/66/r2/final) (HIPAA→800-53). **Indicative and one-way** — implementing an 800-171 control may help satisfy a HIPAA standard, not the reverse. Weak/catch-all mappings are flagged `~`.
  - **HIPAA-only requirements tracker** — the Security Rule requirements 800-171 *doesn't* cover (contingency planning §164.308(a)(7), ePHI integrity, BA-contract content, documentation availability), each individually trackable with faithful CFR text and HIPAA's Required/Addressable classification. **Deliberately kept separate from the SPRS score and the 110-control readiness** — it's a supplement for dual-obligation orgs, not a fake control. The view states plainly that **800-171 compliance is not HIPAA compliance.**
